...
**TourAPI 지역코드 주의** — KorService2 는 옛 `areaCode`(제주 `39`)가 비어 있는 장소가 많다
(성산일출봉도 그렇다). 제주 전체를 받으려면 법정동 코드 `lDongRegnCd=50` 을 쓴다.

## "함께 가면 좋은 곳"(연관 관광지)은 어떻게 돌아가나

장소를 여행에 담으면, 그 장소를 고른 사람들이 많이 간 관광지가 바텀시트로 뜬다. 추천이 없는
장소는 아무것도 안 뜬다. 데이터는 한국관광 데이터랩의 연관 관광지(`TarRlteTarService1`)다.

앱은 이 API 를 **직접 부르지 않는다.** 미리 만들어 커밋해둔 `src/data/relatedIndex.json`
("TourAPI 장소 ID → 추천 장소 ID 목록")만 읽는다. 그래서 pull 받으면 바로 되고, 키도 필요 없다.

이렇게 한 이유: 데이터랩은 장소 **이름만** 주고, TourAPI 와 같은 곳을 다른 이름으로 부른다
("검멀레해수욕장" / "검멀레해변", "주상절리대" / "대포주상절리"). 그래서 두 쪽 제주 데이터를
통째로 받아 스크립트에서 이어 붙인다.

```bash
npm run related:index    # 몇 달에 한 번 — 결과 파일을 커밋한다
```

- `.env.local` 의 `TOUR_API_SERVICE_KEY` 를 쓴다. 그 키로 "관광지별 연관 관광지 정보" 서비스도
  활용신청돼 있어야 한다(팀 키는 돼 있다).
- 데이터랩 지역코드는 TourAPI 와 **다르다** — 제주 `50`, 제주시 `50110`, 서귀포시 `50130`.
  `39` 를 넣으면 에러 없이 0건만 온다.
- 이름 맞추기 규칙과, 규칙으로 못 잡는 예외 목록(`ALIASES`, `NOT_MATCHED`)은 스크립트 맨 위에 있다.
  실행하면 "이름이 달라도 같은 곳으로 이은 것" 목록을 출력하니, 틀린 게 없는지 훑어보고 커밋할 것.
